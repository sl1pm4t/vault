	...
	if len(roleEntry.BoundIamPrincipalARNs) > 0 {
		...
		default:
			...
			if fullArn == "" {
				...
				fullArn, err = b.fullArn(ctx, entity, req.Storage, roleEntry.IAMClientRegion)
				if err != nil {
...
func (b *backend) fullArn(ctx context.Context, e *iamEntity, s logical.Storage, iamClientRegion *string) (string, error) {
	region := ""
